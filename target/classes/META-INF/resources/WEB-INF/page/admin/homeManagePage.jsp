...
<head>
    <script>
        ...
        var barChart;
        $(function () {
            $.getJSON('/minghao/res/js/basicTheme.json', function (themeJSON) {
            ...
            $.getJSON('/minghao/res/js/basicTheme.json', function (themeJSON) {
                echarts.registerTheme('basicTheme', themeJSON);
                // 基于准备好的dom，初始化eCharts实例
                pieChart = echarts.init($("#pieDiv")[0], "basicTheme");
                // 指定图表的配置项和数据
                option = {
                    title: {
                        text: '订单状态',
                ...
                //异步加载数据
                getPieChartData(null, null, JSON.parse('${requestScope.pieObject}'));
            });
            $.getJSON('/minghao/res/js/basicTheme.json', function (themeJSON) {
                echarts.registerTheme('basicTheme', themeJSON);
                // 基于准备好的dom，初始化eCharts实例
                lineChart = echarts.init($("#lineDiv")[0], "basicTheme");
                // 指定图表的配置项和数据
                option = {
                    title: {
                        text: '各类型商品销售记录'
                ...
                //异步加载数据
                getLineChartData(null, null, JSON.parse('${requestScope.lineObject}'));
            });
            $.getJSON('/minghao/res/js/basicTheme.json', function (themeJSON) {
                echarts.registerTheme('basicTheme', themeJSON);
                // 基于准备好的dom，初始化eCharts实例
                barChart = echarts.init($("#barDiv")[0], "basicTheme");
                // 指定图表的配置项和数据
                option = {
                    title: {
                        text: '销售排行榜'
                ...
                //异步加载数据
                getBarChartData(null, null, JSON.parse('${requestScope.barObject}'));
            });
            ...
            $("#btn_chart_search").click(function () {
                ...
                getPieChartData($("#input_beginDate").val(), $("#input_endDate").val(), null);
                getLineChartData($("#input_beginDate").val(), $("#input_endDate").val(), null);
                getBarChartData($("#input_beginDate").val(), $("#input_endDate").val(), null);
            });
            //更改日期时
        ...
        function getChartData(beginDate, endDate, jsonObject) {
            if (jsonObject == null) {
                ...
                //异步加载数据
                myChartDataSet(jsonObject)
            }
        }

        function myChartDataSet(jsonObject) {
            myChart.setOption({
                xAxis: {
        ...
        //获取饼图数据
        function getPieChartData(beginDate, endDate, pieObject) {
            if (pieObject == null) {
                $.ajax({
                    url: "/minghao/admin/home/pie",
                    type: "get",
                    data: {"beginDate": beginDate, "endDate": endDate},
                    dataType: "json",
                    success: function (data) {
                ...
                if (objectIsEmpty(pieObject)) {
                    ...
                    pieChartDataSet(pieObject);
                }

            }
        }

        function pieChartDataSet(pieObject) {
            if (objectIsEmpty(pieObject)) {
                //异步加载数据
                pieChart.setOption({
                    title: {
                    ...
                    series: [
                        {
                            ...
                            data: [
                                {value: pieObject.forThePayment, name: '待付款'},
                                {value: pieObject.toSendTheGoods, name: '待发货'},
                                {value: pieObject.toBeConfirmed, name: '待确认'},
                                {value: pieObject.successfulDeal, name: '订单成功'},
                                {value: pieObject.closeDeal, name: '订单关闭'}
                            ],
                            emphasis: {
                                itemStyle: {
                                    shadowBlur: 10,
        ...
        function getLineChartData(beginDate, endDate, lineObject) {
            if (lineObject == null) {
                $.ajax({
                    url: "/minghao/admin/home/line",
                    type: "get",
                    data: {"beginDate": beginDate, "endDate": endDate},
                    dataType: "json",
                    success: function (data) {
                ...
                lineChartDataSet(lineObject)
            }
        }

        function lineChartDataSet(lineObject) {
            seriesList = [];
            if (lineObject.categoryList.length == 0) {
                lineChart.clear();
                lineChart.setOption({
                    title: {
                        text: '各类型商品销售记录',
            ...
            lineObject.categoryList.forEach((category) => {
                seriesList.push({name: category.name, data: category.count, type: 'line'})
            });
            //异步加载数据
            lineChart.setOption({
                title: {
                    text: "各类型商品销售记录", subtext: ''
                },
                xAxis: {
                    data: lineObject.dateStr
                },
                legend: {
        ...
        function getBarChartData(beginDate, endDate, barObject) {
            if (barObject == null) {
                $.ajax({
                    url: "/minghao/admin/home/bar",
                    type: "get",
                    data: {"beginDate": beginDate, "endDate": endDate},
                    dataType: "json",
                    success: function (data) {
                ...
                barChartDataSet(barObject)
            }
        }

        function barChartDataSet(barObject) {
            if (barObject.productNames.length == 0) {
                barChart.setOption({
                    title: {
                        text: '销售排行榜',
                        subtext: '暂无订单信息'
            ...
            barChart.setOption({
                ...
                xAxis: {
                    type: 'category',
                    axisLabel: {interval: 0, rotate: 30},
                    data: barObject.productNames
                },
                series: [
                    {
                        data: barObject.counts,
                        type: 'bar',
                        showBackground: true,
                        backgroundStyle: {
