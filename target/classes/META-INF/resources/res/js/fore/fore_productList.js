$(function () {
    ...
    $('.context_menu li').click(function () {
        ...
        if ($(this).parent('ul').attr('data-value') === undefined) {
            location.href = '/minghao/product/0/20?orderBy=' + orderBy + "&isDesc=" + isDesc + "&category_id=" + $(this).parent('ul').attr('data-type');
        } else {
            location.href = '/minghao/product/0/20?orderBy=' + orderBy + "&isDesc=" + isDesc + "&product_name=" + $(this).parent('ul').attr('data-value');
        }
    });
    //点击产品图片时
